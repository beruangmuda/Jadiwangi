...
export default function GajiPegawai() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      <StackHeader title="Gaji Pegawai" subtitle="Hitung gaji bulanan per pegawai" />
        ...
        <View style={styles.bar}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: spacing.sm, paddingHorizontal: spacing.lg }}>
            {outlets.map((o: any) => {
              ...
              return (
                <Pressable key={o.id} testID={`gaji-outlet-${o.city}`} onPress={() => setOutletId(o.id)} style={[styles.chip, on && { backgroundColor: colors.brandPrimary, borderColor: colors.brandPrimary }]}>
                  <Text style={[styles.chipText, on && { color: colors.onBrandPrimary }]}>{o.name.replace("Jadiwangi ", "")}</Text>
                </Pressable>
              );
            })}
          </ScrollView>
...
const useStyles = makeStyles((c) => ({
  bar: { paddingVertical: spacing.sm, borderBottomWidth: 1, borderBottomColor: c.divider },
  chip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: radius.pill, borderWidth: 1, borderColor: c.border, backgroundColor: c.surface },
